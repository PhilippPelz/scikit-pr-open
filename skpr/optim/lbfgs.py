...
def lbfgs(opfunc, x, config, state=None):
    ...
    learningRate = config.get('learningRate', 0.1)
    ...
    while nIter < maxIter:
        ...
        if state['nIter'] == 1:
            ...
            torch.mul(d, t, out=s)
            ys = y.view(-1).dot(s.view(-1)).real  # y*s
            if ys > 1e-10:
                # updating memory
                ...
                Hdiag = ys / y.view(-1).dot(y.view(-1)).real  # (y*y)
            else:
            ...
            for i in range(k):
                ro[i] = 1 / old_stps[i].view(-1).dot(old_dirs[i].view(-1)).real
            ...
            for i in range(k - 1, -1, -1):
                al[i] = old_dirs[i].view(-1).dot(q.view(-1)).real * ro[i]
            ...
            for i in range(k):
                be_i = old_stps[i].view(-1).dot(r.view(-1)).real * ro[i]
        ...
        gtd = g.view(-1).dot(d.view(-1)).real  # g * d

        # reset initial guess for step size
        if state['nIter'] == 1:
            tmp1.copy_(g).abs_()
            t = min(1, 1 / tmp1.sum().real) * learningRate
        else:
            t = learningRate
