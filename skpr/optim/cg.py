...
# p['logging'] = Param()
# p['logging']['level'] = 20
# p['logging']['log_file_path'] = 'log.txt'
# io.init_logging(p)
INFINITY = float('inf')
...
def cg(opfunc, x, config, state=None):
    ...
    d1 = -s.view(-1).dot(s.view(-1)).real  # slope
    # print 'd1', d1
    z1 = red / (1 - d1)  # initial step

    print('z1, z2, z3 = %10.3g%10.3g%10.3g' % (z1, z2, z3))
    print('d1, d2, d3 = %10.3g%10.3g%10.3g' % (d1, d2, d3))
    print('f1, f2, f3 = %10.3g%10.3g%10.3g' % (f1, f2, f3))
    ...
    while i < abs(maxEval):
        ...
        df2.copy_(tdf)
        i = i + 1
        d2 = df2.view(-1).dot(s.view(-1)).real
        # print 'd2', d2
        ...
        while True:
            ...
            print('(f2 > f1 + z1 * rho * d1 or d2 > -sig * d1) <==> %10.3g > %10.3g + %10.3g * %10.3g * %10.3g or '
                  '%10.3g > %10.3g * %10.3g  <==>  %10.3g > %10.3g or %10.3g > %10.3g <==> %d or %d <==> %d' % (
                      f2, f1, z1, rho, d1, d2, sig, d1, f2, f1 + z1 * rho * d1, d2, -sig * d1, \
                      int((f2 > f1 + z1 * rho * d1 or d2 > -sig * d1)), int(d2 > -sig * d1), \
                      int(f2 > f1 + z1 * rho * d1 or d2 > -sig * d1)))
        ...
        if success == 1:
            f1 = f2
            fx.append(f1)
            ss = (df2.view(-1).dot(df2.view(-1)).real - df2.view(-1).dot(df1.view(-1)).real) / df1.view(-1).dot(
                df1.view(-1)).real
            s.mul_(ss)
            s.add_(-1, df2)
            tmp = df1.clone()
            df1.copy_(df2)
            df2.copy_(tmp)
            d2 = df1.view(-1).dot(s.view(-1)).real
            if d2 > 0:
                s.copy_(df1)
                s.mul_(-1)
                d2 = -s.view(-1).dot(s.view(-1)).real
