...
function c:simulate_detector(in_psi, mtf, dqe, total_dose)
  ...
  for i=90,110 do
    plt:plot(I[i]:re():fftshift():float(),string.format('I_noise[%d]',i))
    -- plt:plot(I_noise[i]:fftshift():float():log(),string.format('I_noise[%d]',i))
    -- u.printf('sum(I_noise[%d]) = %g',i, I[i]:re():sum())
  end
