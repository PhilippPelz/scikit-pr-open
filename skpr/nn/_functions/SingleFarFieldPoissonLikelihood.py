...
class SingleFarFieldPoissonLikelihood(Function):
    i = 0
    ...
    def forward(ctx, input, I_target, beam_amplitude, valid_mask, gradient_mask, a_h, M, NP, NO):
        ...
        I_model = I_model.squeeze()  # K, M1, M2
        ctx.I_model = I_model
        ...
        # io.plotzmosaic([I,It*mask],'I_model vs I_target',cmap = ['viridis','viridis'], title = ['I','It'])

        out = th.cuda.FloatTensor(I_model.size()[0])
        # print I_model.size(), I_target.size(), valid_mask.size(), out.size()
        for i in range(I_model.size()[0]):
            likelihood(I_model[i], I_target[i], valid_mask[i], out.narrow(0, i, 1))
        io.logger.debug('SingleFarFieldPoissonLikelihood out = %3.2g' % out[0])
        return out.cpu()

    @staticmethod
    def backward(ctx, grad_output):
        io.logger.debug('SingleFarFieldPoissonLikelihood backward 1')
        ...
        io.logger.debug('SingleFarFieldPoissonLikelihood backward 2')
        ...
        #        p.var['gifft'] = fftshift(x[:, 0, 0, :, :],axes=(1,2))
        grad_input.ifft2_()
        gi = grad_input.fftshift((3, 4))

        io.logger.debug('SingleFarFieldPoissonLikelihood backward 3')
